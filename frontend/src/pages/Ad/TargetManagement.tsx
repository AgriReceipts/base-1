...
const TargetManagement: React.FC = () => {
  ...
  const [selectedYear, setSelectedYear] = useState<'2024-25' | '2025-26'>(
    '2025-26'
  );
  ...
  const existingTarget = targets.find(
    (t) =>
      t.committeeId === selectedCommittee && t.financialYear === selectedYear
  );
    ...
    if (value) {
      ...
      if (selectedCommitteeData?.hasCheckposts) {
        ...
        selectedCommitteeData.checkposts.forEach((checkpost) => {
          ...
          months.forEach((month) => {
            newCheckpostTargets[checkpost][month] = (
              monthly / selectedCommitteeData.checkposts.length
            ).toFixed(2);
          });
        });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedCommitteeData?.hasCheckposts) {
      selectedCommitteeData.checkposts.forEach((checkpost) => {
        ...
        months.forEach((month) => {
          checkpostTargetNumbers[checkpost][month] = parseFloat(
            checkpostTargets[checkpost]?.[month] || '0'
          );
        });
      });
    }
    ...
    const newTarget: TargetType = {
      ...
      checkpostTargets: selectedCommitteeData?.hasCheckposts
        ? checkpostTargetNumbers
        : undefined,
    };

    if (existingTarget) {
      setTargets(
        targets.map((t) => (t.id === existingTarget.id ? newTarget : t))
      );
    } else {
  ...
  const loadExistingTarget = (target: TargetType) => {
    ...
    months.forEach((month) => {
      monthlyTargetStrings[month] =
        target.monthlyTargets[month]?.toString() || '0';
    });
    ...
    if (target.checkpostTargets) {
      ...
      Object.keys(target.checkpostTargets).forEach((checkpost) => {
        ...
        months.forEach((month) => {
          checkpostTargetStrings[checkpost][month] =
            target.checkpostTargets![checkpost][month]?.toString() || '0';
        });
      });
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            ...
            <div>
              ...
              <select
                value={selectedYear}
                onChange={(e) =>
                  setSelectedYear(e.target.value as '2024-25' | '2025-26')
                }
                className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500'>
