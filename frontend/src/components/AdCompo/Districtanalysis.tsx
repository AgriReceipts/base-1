...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, subtitle, color }) => {
  return (
    <div className={`bg-white rounded-lg shadow-md p-4 border-l-4`} style={{ borderLeftColor: color }}>
      ...
      <p className="text-2xl font-bold my-2" style={{ color }}>{value}</p>
...
const LineChartComponent: React.FC<LineChartProps> = ({ data, dataKey }) => {
  const maxValue = Math.max(...data.map(item => item[dataKey] as number || 0));
  
  return (
    <div className="w-full overflow-x-auto">
      <svg width="100%" height="300" viewBox="0 0 800 300" className="min-w-[800px]">
        {/* Grid Lines */}
        ...
        {/* X Axis */}
        <line x1="50" y1="250" x2="750" y2="250" stroke="#d1d5db" strokeWidth="1.5" />
        {data.map((item, index) => (
          <text 
            key={`xaxis-${index}`} 
        ...
        {/* Y Axis */}
        <line x1="50" y1="50" x2="50" y2="250" stroke="#d1d5db" strokeWidth="1.5" />
        ...
        {/* Area */}
        <path
          d={`M50,250 ${data.map((item, index) => 
            `L${50 + (700 / (data.length - 1)) * index},${250 - ((item[dataKey] as number || 0) * (200 / maxValue))}`
          ).join(' ')} L750,250 Z`}
          ...
          points={
            data
              .map((item, index) => {
                ...
                const y = 250 - ((item[dataKey] as number || 0) * (200 / maxValue));
                return `${x},${y}`;
              })
              .join(' ')
          }
        ...
        {/* Legend */}
        <text x="700" y="30" textAnchor="end" className="text-xs fill-gray-600 font-medium">
          Achievement (₹ Lakhs)
        </text>
      </svg>
    </div>
    ...
    <div className="w-full overflow-x-auto">
      <svg width="100%" height="300" viewBox="0 0 800 300" className="min-w-[800px]">
        ...
        {data.map((item, index) => {
          ...
          const previousYearHeight = (item.previousYear || 0) * (200 / maxValue);
        ...
        <text x="620" y="30" className="text-xs fill-gray-600">Current Year</text>
        <rect x="600" y="40" width="15" height="15" fill="#10b981" rx="2" />
        <text x="620" y="50" className="text-xs fill-gray-600">Previous Year</text>
      </svg>
    </div>
  );
};
...
const PieChartComponent: React.FC<PieChartProps> = ({ data }) => {
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg width="100%" height="300" viewBox="0 0 300 300" className="min-w-[300px]">
        {/* Pie Slices */}
        {data.map((item, index) => {
          ...
          const pathData = [
            ...
            `A 100 100 0 ${largeArcFlag} 1 ${150 + endX * 100} ${150 + endY * 100}`,
            'Z'
          ].join(' ');
  ...
  return (
    <div className="w-full overflow-x-auto">
      ...
      >
        {/* Bars */}
        {data.map((item, index) => (
          <g key={`bar-${index}`} transform={`translate(0, ${index * 40 + 20})`}>
            {/* Bar Label */}
            <text 
              x="0" 
              y="15" 
...
const HeatmapComponent: React.FC<HeatmapComponentProps> = ({ data, getColor }) => {
      ...
      >
        ...
        {committees.map((committee, index) => (
          <text 
            ...
            className="text-xs fill-gray-600"
          >
            {committee.length > 20 ? `${committee.substring(0, 17)}...` : committee}
          </text>
        ))}
        ...
        {[0, 20, 40, 60, 80, 100].map((value, index) => (
          <g 
            key={`legend-${index}`} 
            transform={`translate(${250 + index * 50}, ${committees.length * 30 + 60})`}
          >
            <rect 
              x="0" 
              y="0" 
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Target (₹ Lakhs)</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Achieved (₹ Lakhs)</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.committee}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.target.toLocaleString()}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.achieved.toLocaleString()}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.achievementPercentage}%</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.receipts.toLocaleString()}</td>
...
const DistrictAnalysis: React.FC = () => {
  ...
  const generateMockData = () => {
    ...
    const committees = ['Market Committee', 'Transport Committee', 'Trade Committee', 'Agriculture Committee', 'Industrial Committee'];
    ...
    const committeeTableData = committees.map((committee, index) => ({
      ...
      status: ['On Track', 'Needs Attention', 'Critical'][Math.floor(Math.random() * 3)]
    }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6">District Revenue Analysis Dashboard</h1>

        {/* Filters & Controls */}
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          <h2 className="text-lg font-semibold text-gray-700 mb-4">Filters & Controls</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-1">
              <label htmlFor="committee" className="block text-sm font-medium text-gray-700">Committee</label>
              <select
                id="committee"
                name="committee"
              ...
              >
                ...
                <option value="Industrial Committee">Industrial Committee</option>
              </select>
            </div>
        ...
        <div className="mb-6">
          <ChartCard title="Top Checkpost Performance (in Lakhs)" className="w-full">
        ...
        {/* Heatmap */}
        <div className="mb-6">
          <ChartCard title="Committee-Month Performance Heatmap" className="w-full">
