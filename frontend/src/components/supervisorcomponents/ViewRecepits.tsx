...
const ViewReceipts: React.FC = () => {
  ...
  const filteredReceipts = receipts.filter((receipt) => {
    ...
    const matchesSearch =
      receipt.traderName.toLowerCase().includes(normalizedSearch) ||
      receipt.payeeName.toLowerCase().includes(normalizedSearch) ||
      receipt.bookNumber.toLowerCase().includes(normalizedSearch) ||
      receipt.nature.toLowerCase().includes(normalizedSearch) ||
      receipt.receiptDate.includes(normalizedSearch) ||
      receipt.value.toString().includes(normalizedSearch) ||
      receipt.collectedBy.toLowerCase().includes(normalizedSearch) ||
      receipt.designation.toLowerCase().includes(normalizedSearch);
      ...
      <div className="overflow-x-auto rounded-2xl shadow-md bg-white">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-blue-50">
            <tr>
              {["Date", "Book/Receipt", "Trader", "Payee", "Amount Paid", "Nature", "Generated By", "Designation", "Actions"].map((heading, idx) => (
                <th
                  key={idx}
                  className={`px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider ${
                    idx === 0 ? 'rounded-tl-2xl' : idx === 8 ? 'text-right rounded-tr-2xl' : ''
                  }`}
                >
                  {heading}
                </th>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {filteredReceipts.length > 0 ? (
              filteredReceipts.map((receipt) => (
                ...
                >
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.receiptDate}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.bookNumber}/{receipt.receiptNumber}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.traderName}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.payeeName}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">₹ {Number(receipt.value).toLocaleString()}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.nature}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.collectedBy}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{receipt.designation}</td>
